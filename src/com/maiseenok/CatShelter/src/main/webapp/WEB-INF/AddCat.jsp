			...
			<c:forEach items="${ catDad }" var="catD">
				<c:choose>
					<c:when test="${catD.id eq cat.dad.id }">
						<option value="${ catD.id }" selected="selected">${ catD.name } (id ${ catD.name })</option>
					</c:when>
					<c:otherwise>
						<c:if test="${ catD.id != cat.id}">
							<option value="${ catD.id }">${ catD.name } (id ${ catD.name })</option>
						</c:if>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:forEach items="${ catMom }" var="catM">
				<c:choose>
					<c:when test="${catM.id eq cat.mom.id }">
						<option value="${ catM.id }" selected="selected">${ catM.name } (id ${ catM.name })</option>
					</c:when>
					<c:otherwise>
						<c:if test="${ catM.id != cat.id}">
							<option value="${ catM.id }">${ catM.name } (id ${ catM.name })</option>
						</c:if>
					</c:otherwise>
				</c:choose>
			</c:forEach>
